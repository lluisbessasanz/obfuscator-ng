...
unsigned llvm::scramble32(const unsigned in, const char key[16]) {
  assert(key != NULL);
...
static bool llvm::valueEscapes(llvm::Instruction &Inst) {
  llvm::BasicBlock *BB = Inst.getParent();
  for (const llvm::User *U : Inst.users()) {
    auto *I = llvm::dyn_cast<llvm::Instruction>(U);
    if (!I || I->getParent() != BB || llvm::isa<llvm::PHINode>(I))
      return true;
  }
  return false;
}
...
void llvm::fixStack(Function &F) {
  ...
  do {
    ...
    for (Function::iterator i = F.begin(); i != F.end(); ++i) {
      ...
      for (BasicBlock::iterator j = i->begin(); j != i->end(); ++j) {
        ...
        if (!(isa<AllocaInst>(j) && j->getParent() == bbEntry) &&
            (llvm::valueEscapes(*j) || j->isUsedOutsideOfBlock(&*i))) {
          tmpReg.push_back(&*j);
          continue;
        }
      }
    ...
    for (unsigned int i = 0; i != tmpReg.size(); ++i) {
      llvm::DemoteRegToStack(*tmpReg.at(i), false, bbEntry->begin());
    }

    for (unsigned int i = 0; i != tmpPhi.size(); ++i) {
      llvm::DemotePHIToStack(tmpPhi.at(i), bbEntry->begin());
    }
